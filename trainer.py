...
class Trainer(object):
    ...
    def train_epoch(self):
        ...
        for i, (v_d, v_f, v_p, labels) in enumerate(tqdm(self.train_dataloader)):
            self.step += 1
            v_d, v_f, v_p, labels = v_d.to(self.device), v_f.to(self.device), v_p.to(self.device), labels.float().to(
                self.device)
            self.optim.zero_grad()
            v_d, v_p, f, score = self.model(v_d, v_f, v_p)  # 传递 v_f (分子指纹)
            if self.n_class == 1:
    ...
    def train_da_epoch(self):
        ...
        for i, (batch_s, batch_t) in enumerate(tqdm(self.train_dataloader)):
            self.step += 1
            v_d, v_f, v_p, labels = batch_s[0].to(self.device), batch_s[1].to(self.device), batch_s[2].to(self.device), \
            batch_s[3].float().to(self.device)
            v_d_t, v_f_t, v_p_t = batch_t[0].to(self.device), batch_t[1].to(self.device), batch_t[2].to(self.device)
            self.optim.zero_grad()
            self.optim_da.zero_grad()
            v_d, v_p, f, score = self.model(v_d, v_f, v_p)  # 传递分子指纹 v_f
            if self.n_class == 1:
                n, model_loss = binary_cross_entropy(score, labels)
            else:
                n, model_loss = cross_entropy_logits(score, labels)
            if self.current_epoch >= self.da_init_epoch:
                v_d_t, v_f_t, v_p_t, f_t, t_score = self.model(v_d_t, v_f_t, v_p_t)  # 传递目标域的分子指纹 v_f_t
    ...
    def test(self, dataloader="test"):
        ...
        with torch.no_grad():
            self.model.eval()
            for i, (v_d, v_d_fingerprint, v_p, labels) in enumerate(data_loader):
                v_d, v_d_fingerprint, v_p, labels = v_d.to(self.device), v_d_fingerprint.to(self.device), v_p.to(
                    self.device), labels.float().to(self.device)
                if dataloader == "val":
                    v_d, v_p, f, score = self.model(v_d, v_d_fingerprint, v_p)
                elif dataloader == "test":
                    v_d, v_p, f, score = self.best_model(v_d, v_d_fingerprint, v_p)
                if self.n_class == 1:
                    n, loss = binary_cross_entropy(score, labels)
                else:
                    n, loss = cross_entropy_logits(score, labels)
